  ...
  <div class="space-y-6">
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        ...
        <div v-if="!loading && roles.length" class="text-sm text-gray-500 dark:text-gray-400">
          共 {{ pagination.totalElements }} 个角色
        </div>
      </div>
      ...
      <div v-else-if="!roles.length" class="text-center py-12">
        <div class="text-gray-500 dark:text-gray-400">
          ...
          <p class="text-lg font-medium mb-2">暂无角色</p>
          <p class="text-sm">点击"新建角色"按钮创建第一个角色</p>
        </div>
      </div>
          ...
          <div class="p-6">
            ...
            <div class="mb-4 space-y-2">
              <div class="text-sm text-gray-600 dark:text-gray-400">
                <span class="font-medium">代码:</span> {{ role.code }}
              </div>
              <div v-if="role.description" class="text-sm text-gray-600 dark:text-gray-400 line-clamp-2">
                <span class="font-medium">描述:</span> {{ role.description }}
              </div>
            </div>
...
const changePage = (page: number) => {
  if (page >= 0 && page < pagination.value.totalPages) {
    roleStore.updateQueryParams({ page })
    loadRoles()
  }
}
...
// 监听查询参数变化
watch(
  () => roleStore.queryParams,
  () => {
    searchKeyword.value = roleStore.queryParams.keyword || ''
  },
  { immediate: true }
)
